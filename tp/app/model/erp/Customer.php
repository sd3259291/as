...
use think\facade\Db;
use think\Model;
use app\validate\erp\CustomerValidate;
...
class Customer extends Model{
	...
	public function getCustomer($post){
		...
		$r = Db::query("select count(1) n from s_Customer v where $w");
		$page = array();
        $page['totles'] = $r[0]['n'];     //总记录数 返回
        $page['totle_page'] = ceil($r[0]['n'] / $post['n']);  //总页数 返回
        $page['n'] = $post['n'];
        $page['current_page'] = $post['page'];     //当前页 返回
		$r = Db::table('s_Customer')->alias('v')->join(['s_basic_class' => 'b'],'v.basic_class_id = b.id')->where($w)->field('v.code,v.name,v.contact,v.phone,v.email,v.tel,v.fax')->order('v.code asc')->page($post['page'],$post['n'])->select()->toArray();
		$tbody = "";
		foreach($r as $k => $v){
	...
	public function getCustomerByCode($get){
